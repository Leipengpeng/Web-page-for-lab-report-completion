...
from controller.PDFocr import PDFocr
from controller.chatGPT import purify, program, homeWorkD, out, short
from entity.languageForm import languageForm
...
def homeWorkDown():
    ...
    if request.method == 'POST':
        ...
        language_select = form.language_select.data
        output, outputP, outputO, outputH = homeWork(file_path, language_select)
...
# chatGPT OCR逻辑
def homeWork(file_path, name):
    if request.method == 'POST':
        # output OCR结果 实验描述
        output = PDFocr(file_path)

        # outputP 代码段
        outputP = program("以下内容基于"+name+"语言。"+output)

        # outputO 代码伪运行 outputS简短化的实验描述
        outputS = short("以下内容基于"+name+"语言。"+output)
        outputO = out("以下内容基于"+name+"语言。"+outputS, outputP)
        front = r"C:\Users\WHUT\AppData\Local\Programs\Python\Python310\python.exe " \
                r"C:/Users/WHUT/Desktop/homeWork/main.py "
        end = r"进程已结束,退出代码0"
        outputO = front + "\n" + outputO + "\n" + end

        # outputH 实验报告
        outputH = homeWorkD("以下内容基于"+name+"语言。"+output)
        return output, outputP, outputO, outputH
